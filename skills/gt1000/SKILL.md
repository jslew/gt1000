...
- STOMPBOX questions: explain the user-facing caution from `references/gt1000-wiki/owner-manual.md`; use `patch stompbox --live` only when raw PatchStompBox bytes are explicitly useful for investigation. Do not infer shared STOMPBOX selection semantics from raw bytes yet.
