...
- For reference-tone matching/chasing, the agent owns the workflow. Ask for or locate the isolated reference WAV and a dry take/session, run the USB audio lab internally, then report the ranked audition results in musician-facing language. Do not make the user run command lines, and do not save a candidate to a user slot unless they explicitly approve a named slot. If a candidate uses a parameter that is not already live-verified, verify the write/read-back before rendering; skip and report any candidate that fails verification.
